...
| `System.IO` | expanded generated interface with native standard-handle text IO | Haskell 2010 `System.IO` exports are importable, including `IOMode(..)`, `BufferMode(..)`, `SeekMode(..)`, `Handle`, `HandlePosn`, standard handles, open/close, buffering, position, handle property, text input/output, Prelude IO aliases, `readIO`, and `readLn`. Native execution is covered for standard-handle text IO, buffering calls, `hShow`, `hPrint`, line input, `getContents`, and EOF checks. File-backed handle storage, lazy semi-closed `hGetContents`, real seek/position state, and `fixIO` remain strict-runtime deviations documented under `LIB-012`. |
