...
2. RTS-009 — process-lifetime arena or GC decision: Set the memory-management direction for native lazy runtime objects.
3. RTS-019 — runtime leak/ownership documentation: Document ownership guarantees before expanding runtime allocation pressure.
4. ADT-004 — newtype representation: Define runtime/Core representation for `newtype`.
5. ADT-005 — record field labels: Add the name-resolution and selector surface required for records.
6. PAT-008 — irrefutable/lazy patterns: Implement lazy pattern semantics rather than only parsed/renamed syntax.
7. PAT-014 — exhaustiveness warning placeholder: Establish the diagnostic placeholder for later pattern coverage checking.
8. CORE-REC-004 — recursive pattern bindings: Desugar recursive pattern bindings through the Core recursion model.
9. PRELUDE-DATA-006 — Char runtime representation: Finish native/runtime treatment for `Char`.
10. PRELUDE-DATA-007 — String = [Char]: Align source strings with list-of-Char semantics.
11. PRELUDE-DATA-008 — arithmetic sequences: Implement the `Enum`-driven sequence surface.
12. PRELUDE-DATA-009 — list comprehensions: Desugar list comprehensions into the supported list/Core subset.
13. PRELUDE-DATA-012 — String literal native wet tests: Broaden native tests for source strings and printed strings.
14. TC-003 — superclass representation: Model superclass relationships before broader class solving.
15. TC-005 — default methods: Implement default class method typing and dictionary filling.
16. TC-008 — overlapping instance rejection per Haskell 2010: Reject overlapping/duplicate instance choices before broader instance search.
17. TC-015 — Show: Finish the supported `Show` surface beyond the current built-in exact instances.
18. TC-016 — Read, if implemented or documented deviation: Decide and document whether `Read` enters the supported class surface.
19. TC-018 — Enum: Implement or explicitly defer the Haskell 2010 `Enum` class surface.
20. TC-019 — Bounded: Implement or explicitly defer the Haskell 2010 `Bounded` class surface.
